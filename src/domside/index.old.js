export default function (parentClass) {
  return class FMODManager extends parentClass {
      ...
      try {
        ...
        const resumeAudio = (realTry = true) => {
          if (!this.gAudioResumed) {
            try {
              // Don't call OutputAudioWorklet_resumeAudio during init - it triggers mixer functions
              // that cause critical section warnings before FMOD is fully initialized
              if (this._fullyInitialized) {
                this.FMOD["OutputAudioWorklet_resumeAudio"]();
              }

              // Only call mixer suspend/resume after full initialization to prevent critical section warnings
              if (this._fullyInitialized && this.gSystemCore) {
        ...
        // Give FMOD a moment to fully initialize its internal state
        // This prevents "NULL critical section" warnings
        setTimeout(() => {
          this._fullyInitialized = true;
          // Now it's safe to call resumeAudio
          resumeAudio(false);
        }, 100);
    ...
    update() {
      // Skip update if system isn't ready
      if (!this.banks || !this.gSystem || !this.gSystemCore || !this._loaded) {
        return;
      }

      // Skip early updates to prevent critical section warnings
      if (!this._fullyInitialized) {
        return;
      }
      ...
      try {
        ...
        // Clean up invalid instances periodically (every 60 updates ~ 1 second)
        if (!this._updateCounter) this._updateCounter = 0;
        this._updateCounter++;
        if (this._updateCounter % 60 === 0) {
          this._cleanupInvalidInstances();
        }

        // Update FMOD - only if system is fully initialized
    ...
    // Check if an FMOD instance is still valid (not deleted/released)
    _isInstanceValid(instance) {
      if (!instance) return false;

      try {
        // Try to get playback state - if instance is deleted, this will throw
        const outval = {};
        const result = instance.getPlaybackState(outval);
        // If we get ERR_INVALID_HANDLE, the instance was deleted
        return result !== this.FMOD.ERR_INVALID_HANDLE;
      } catch (error) {
        // If any error occurs (including "Cannot use deleted val"), instance is invalid
        return false;
      }
    }

    // Periodically clean up invalid instances to prevent memory leaks and update errors
    _cleanupInvalidInstances() {
      try {
        Object.keys(this.events).forEach((eventName) => {
          const event = this.events[eventName];

          // Clean up allInstances array
          const validInstances = event.allInstances.filter((instance) =>
            this._isInstanceValid(instance)
          );

          // Only update if we found invalid instances
          if (validInstances.length !== event.allInstances.length) {
            event.allInstances = validInstances;

            // Clean up tag mappings
            event.instance.forEach((instances, tag) => {
              const validTagInstances = instances.filter((instance) =>
                this._isInstanceValid(instance)
              );
              if (validTagInstances.length !== instances.length) {
                event.instance.set(tag, validTagInstances);
              }
            });
          }
        });
      } catch (error) {
        // Don't let cleanup errors break the update cycle
        console.error(
          "FMOD [_cleanupInvalidInstances]: Error during cleanup:",
          error
        );
      }
    }
      ...
      try {
        ...
        instancesInTag.forEach((instance) => {
          try {
            ...
            // Release after starting - FMOD will clean up when sound finishes
            if (destroyWhenStopped) {
              this.assert(instance.release());
              instancesToRelease.push(instance);
            }
          } catch (error) {
            console.error(
        ...
        // Clean up references for released instances
        if (instancesToRelease.length > 0) {
          this.nextTick(() => {
            // Remove released instances from tracking
            this.events[event].allInstances = this.events[
              event
            ].allInstances.filter(
              (instance) => !instancesToRelease.includes(instance)
            );
    ...
    stopEvent(event, tag, allowFadeOut, release) {
      ...
      const instancesToRelease = [];
      instancesInTag.forEach((instance) => {
        // Skip if instance is already invalid
        if (!this._isInstanceValid(instance)) {
          instancesToRelease.push(instance);
          return;
        }
      ...
      if (release || instancesToRelease.length > 0) {
        this.events[event].allInstances = this.events[
          event
        ].allInstances.filter(
          (instance) => !instancesToRelease.includes(instance)
        ...
        const validInstances =
          this.events[event].instance
            .get(tag)
            ?.filter((instance) => !instancesToRelease.includes(instance)) ||
          [];
        this.events[event].instance.set(tag, validInstances);
      }
    }
