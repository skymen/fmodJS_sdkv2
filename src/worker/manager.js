...
// FMOD lives in a dedicated worker (see index.js in this folder) so that
// main-thread stalls can't starve the mixer. `parentClass` only has to
// provide AddRuntimeMessageHandlers([[name, fn], ...]).
export default function (parentClass) {
  return class FMODManager extends parentClass {
    ...
    constructor(options = {}) {
      ...
      // Library selection (see LoadLibrary)
      this.libDebug = false;
    ...
    // Picks which build of fmodstudio to run: release, or the logging build
    // when the Debug property is on. Decided here (not at build time) so a
    // single addon can ship both and only the chosen one is ever fetched.
    SelectLibrary() {
      return {
        name: "fmodstudio" + (this.libDebug ? "L" : ""),
        debug: this.libDebug,
      };
    }
    ...
    LoadLibrary() {
      ...
      try {
        ...
        console.error(`FMOD: failed to load library ${url}`, error);
      }
    }

    OnUserInteraction(real) {
      if (this.wrapper) this.wrapper.onUserInteraction(real);
    }
    ...
    async onRuntimeInitialized() {
      try {
        ...
        // Initial resume attempt; real gestures arrive via OnUserInteraction
        this.wrapper.onUserInteraction(false);
    ...
    async unloadBank(bankOrName) {
      ...
      try {
        // Bank::unload invalidates the handle straight away (the actual
        // unload finishes during the next update), so there is nothing to
        // poll afterwards.
        this.assert(bankOrName.bankHandle.unload());

        bankOrName.loaded = false;
        bankOrName.bankHandle = null;
